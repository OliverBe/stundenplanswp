...
public class Planungseinheit {
	...
	private boolean pendelCheck = false;
	...
	public Planungseinheit(int pId, Weekday pDay, int pStartHour,
			int pStartMin, int pEndHour, int pEndMin) {
		id = pId;
		day = pDay;
	...
	public void setWeekday(Weekday pWeekday) {
		if (pWeekday == null) {
			new IllegalArgumentException("Argument must be not null");
		}
		day = pWeekday;
	}
	...
	public Schoolclass getSchoolclassByName(final String pName) {
		...
		for (String name : schulklassen) {
			if (name.equals(pName))
				return DataSchulklasse.getSchulklasseByName(name);
		}
		return null;
	}
	...
	public Room getRoomByName(final String pName) {
		...
		for (String name : raeume) {
			if (name.equals(pName))
				return DataRaum.getRaumByName(name);
		}
		return null;
	}
	...
	/*
	 * gibt Personal fuer eine Name oder Kuerzel zurï¿½ï¿½ck.
	 */
	public Personal getPersonalbyKuerzel(final String pKuerzel) {
		if (pKuerzel == null || pKuerzel.length() <= 0) {
	...
	public Personal getPersonalbyIndex(final int pIndex) {
		if (pIndex < 0) {
			new IllegalArgumentException("Argument must not be less than zero");
		}
	...
	public boolean containsPersonal(final Personal pPerson) {
		...
		if (personal.get(pPerson.getKuerzel()) != null) {
			return true;
		}
		return false;
	}
	...
	public boolean isWeekday(Weekday pWeekday) {
		...
		if (day.getOrdinal() == pWeekday.getOrdinal()) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Methode nur von Planungseinheitmanager.getPEforPersonalAndWeekday zu
	 * nutzen setzt die Zeiten der PE mit der Eingabe eines 4-elementigen Array.
	 * 
	 * @param time
	 */
	...
	public void setStarthour(final int pStarthour) {
		if (pStarthour < 0) {
			throw new IllegalArgumentException(
					"Argument must not be less than 0");
		}
		startHour = pStarthour;
	}
	 ...
	 * 
	 * @return gib die Dauer der Planungseinheit in Minuten zurï¿½ï¿½ck
	 */
	public int duration() {
		int dur = TimetableManager.duration(startHour, startMin, endHour,
				endMin);
		return dur;
	}
	...
	/**
	 * für die PendelZeit wird für den StundenplanPanel geprüft ob diese
	 * Planungseinheit überprüft werden soll. True zeigt dass es
	 * ignoriert wird
	 */
	public void setPendelCheck(boolean state){
		pendelCheck = state;
	}
	public boolean getPendelCheck() {
		return pendelCheck;
	}
}
